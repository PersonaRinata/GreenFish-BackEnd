...
import (
	...
	"GreenFish/server/kitex_gen/aigc/aigcserver"
...
func initAIGC() {
	...
	r := nacos.NewNacosResolver(nacosCli, nacos.WithGroup("AIGC_GROUP"))
	if err != nil {
	...
	c, err := aigcserver.NewClient(
		...
		client.WithClientBasicInfo(&rpcinfo.EndpointBasicInfo{ServiceName: config.GlobalServerConfig.AIGCSrvInfo.Name}),
	)
	if err != nil {
		klog.Fatalf("ERROR: cannot init client: %v\n", err)
	}
	config.GlobalAIGCClient = c
}
